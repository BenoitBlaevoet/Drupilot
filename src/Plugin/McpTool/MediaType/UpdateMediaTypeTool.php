...
/**
 * Updates the label or description of an existing custom media type.
 */
#[McpTool(
  id: 'media_type_update',
  label: 'Update media type',
  description: 'Updates the label or description of an existing custom media type.',
  category: 'media_type',
)]
final class UpdateMediaTypeTool implements McpToolInterface {
  ...
  public function getInputSchema(): array {
    return [
      ...
      'properties' => [
        'machine_name' => [
          'type' => 'string',
          'description' => 'Machine name of the existing media type to update.',
        ],
        'label' => [
          'type' => 'string',
          'description' => 'New human-readable label for the media type.',
        ],
        'description' => [
          'type' => 'string',
          'description' => 'New description for the media type.',
        ],
      ],
    ...
    try {
      ...
      if (array_key_exists('source', $input)) {
        return McpResponse::error(
          ...
          'Media source cannot be changed after creation.',
        );
      }
      ...
      if ($label !== NULL) {
        $mediaType->set('label', $label);
      }

      $description = $this->getOptionalString($input, 'description');
      if ($description !== NULL) {
        $mediaType->set('description', $description);
      }

      $mediaType->save();

      $this->logger->info(
        'MCP: Updated media type @machine_name.',
