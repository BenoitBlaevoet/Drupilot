...
#[McpTool(
  id: 'role_create',
  label: 'Create role',
  description: 'Creates a new Drupal role with the given machine name and label.',
  category: 'role',
)]
final class CreateRoleTool implements McpToolInterface {
  ...
  public function getInputSchema(): array {
    return [
      ...
      'properties' => [
        'machine_name' => [
          'type' => 'string',
          'description' => 'Machine name of the role (lowercase letters, numbers, underscores only).',
          'pattern' => '^[a-z0-9_]+$',
        ],
        'label' => [
          'type' => 'string',
        ...
        'weight' => [
          'type' => 'integer',
          'description' => 'Weight for ordering roles.',
        ],
      ],
  ...
  public function execute(array $input): McpResponse {
    try {
      ...
      $weight = $this->getOptionalInt($input, 'weight') ?? 0;
      ...
      if ($existing !== NULL) {
        return McpResponse::error(
          ...
          sprintf("Role '%s' already exists.", $machineName),
        );
      }
      ...
      $this->logger->info(
        'MCP: Created role @machine_name (@label).',
